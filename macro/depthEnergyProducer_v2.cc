...
pfCond_Token_(consumes<edm::View<reco::PFCandidate>>(iConfig.getParameter<edm::InputTag>("pfCondLabel"))),
...
//  produces<edm::ValueMap<float> >("");
  //produces<edm::View<edm::ValueMap<std::vector<float>>> >("");
...
////////////////
  ...
  //const std::vector<reco::PFCandidate>& pfcondidates_ = *pfcondidates; 
   ...
   for (unsigned int icl = 0; icl < nclust; ++icl) {
      ...
      Ptr<reco::PFCandidate> pcand = pfcondidates->ptrAt( icl );//pfClusters.ptrAt( icl );
     ...
     for (auto & hitRefAndFrac : pfClusters[icl].recHitFractions()) {
        const auto & hit = *hitRefAndFrac.recHitRef();
          ...
          //std::cout << "depth per hit:   "  << hit.depth()  << std::endl;  
